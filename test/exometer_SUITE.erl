...
init_per_testcase(Case, Config)
    when Case == test_folsom_histogram
    orelse Case == test_history1_folsom
    orelse Case == test_history4_folsom ->
...
init_per_testcase(Case, Config)
    when Case == test_ext_predef
    orelse Case == test_function_match ->
    Scr = filename:join(?config(data_dir, Config), "test_defaults.script"),
...
init_per_testcase(test_app_predef, Config) ->
    ...
    {ok, StartedApps} = application:ensure_all_started(exometer_core),
    ct:log("StartedApps = ~p~n", [StartedApps]),
    Scr = filename:join(?config(data_dir, Config), "app1.script"),
...
test_aggregate(_Config) ->
    ...
    [{value, 12}] = exometer:aggregate(
        [{{[?MODULE, K, a, '_'], '_', '_'}, [], [true]}], [value]),
    [{50, 1}, {75, 1}, {90, 1}, {95, 1}, {99, 1}, {999, 1},
        {max, 1}, {mean, 1}, {median, 1}, {min, 1},
        {ms_since_reset, _}, {n, _}, {value, 12}] =
        exometer:aggregate([{{[?MODULE, K, '_', '_'], '_', '_'}, [], [true]}], default),
    ok.

test_history1_slide(Config) ->
    test_history(1, slide, filename:join(
        ?config(data_dir, Config), "puts_time_hist1.bin")).

test_history1_slotslide(Config) ->
    test_history(1, slot_slide, filename:join(
        ?config(data_dir, Config), "puts_time_hist1.bin")).

test_history1_folsom(Config) ->
    test_history(1, folsom, filename:join(
        ?config(data_dir, Config), "puts_time_hist1.bin")).

test_history4_slide(Config) ->
    test_history(4, slide, filename:join(
        ?config(data_dir, Config), "puts_time_hist4.bin")).

test_history4_slotslide(Config) ->
    test_history(4, slot_slide, filename:join(
        ?config(data_dir, Config), "puts_time_hist4.bin")).

test_history4_folsom(Config) ->
    test_history(4, folsom, filename:join(
        ?config(data_dir, Config), "puts_time_hist4.bin")).
...
test_app_predef(Config) ->
    ...
    File = filename:join(?config(data_dir, Config), "app1_upg.script"),
...
compile_app1(Config) ->
    ...
    Dir = filename:join(DataDir, "app1"),
    ct:log("Dir = ~p~n", [Dir]),
